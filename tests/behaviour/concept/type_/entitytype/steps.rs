...
async fn get_entity_type(tx: &Transaction<'_>, type_label: String) -> TypeDBResult<EntityType> {
...
generic_step_impl! {
    ...
    async fn put_entity_type(context: &mut Context, type_label: String) -> TypeDBResult<EntityType> {
        context.transaction().concept().put_entity_type(type_label).await
    }

    #[step(expr = "delete entity type: {word}")]
    async fn delete_entity_type(context: &mut Context, type_label: String) -> TypeDBResult {
        let tx = context.transaction();
        get_entity_type(tx, type_label).await?.delete(tx).await
    }

    #[step(expr = "delete entity type: {word}; throws exception")]
    async fn delete_entity_type_throws(context: &mut Context, type_label: String) {
    ...
    async fn entity_type_set_owns_attribute_type_with_annotations_throws(
        ...
        assert!(entity_type_set_owns_attribute_type_with_annotations(
            context,
            type_label,
