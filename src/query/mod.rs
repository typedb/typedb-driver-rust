...
use crate::rpc::builder::query_manager::{
    define_req, delete_req, insert_req, match_aggregate_req, match_req, undefine_req, update_req,
};
...
macro_rules! stream_concept_maps {
    ($self:ident, $req:ident, $res_part_kind:ident, $query_type_str:tt) => {
        $self
            .stream_answers($req)
            .flat_map(
                |result: Result<query_manager::res_part::Res>| match result {
                    Ok(res_part) => match res_part {
                        $res_part_kind(x) => {
                            stream::iter(x.answers.into_iter().map(|cm| ConceptMap::from_proto(cm)))
                                .left_stream()
                        }
                        _ => stream::iter(once(Err(MESSAGES
                            .client
                            .missing_response_field
                            .to_err(vec![format!(
                                "query_manager_res_part.{}_res_part",
                                $query_type_str
                            )
                            .as_str()]))))
                        .right_stream(),
...
impl QueryManager {
    ...
    pub async fn match_aggregate(&mut self, query: &str) -> Result<Numeric> {
        match self.single_call(match_aggregate_req(query, None)).await? {
            ...
            _ => Err(MESSAGES
                .client
                .missing_response_field
                .to_err(vec!["match_aggregate_res"])),
        }
    }
    ...
    pub async fn match_aggregate_with_options(
        ...
        match self
            .single_call(match_aggregate_req(query, Some(options.to_proto())))
            .await?
        {
            MatchAggregateRes(res) => res.answer.unwrap().try_into(),
            _ => Err(MESSAGES
        ...
        match self.tx.single(req).await?.res {
            Some(transaction::res::Res::QueryManagerRes(res)) => res.res.ok_or_else(|| {
                MESSAGES
                    .client
                    .missing_response_field
                    .to_err(vec!["res.query_manager_res"])
            }),
            _ => Err(MESSAGES
                .client
                .missing_response_field
    ...
    fn stream_answers(
        ...
        self.tx
            .stream(req)
            .map(|result: Result<transaction::ResPart>| match result {
